...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        for k, v in index.items():
            for i in v:
                calc = ((1.0 + math.log10(i[1])) * \
                        (math.log10(n_docs/float(doc_freqs[k]))))**2.0 
                d[i[0]] += calc
    ...
    def compute_doc_lengths(self, index):
        ...
        for i in index:
            ...
            for j in line:
                doc, term = j[0], j[1]
                doc, term = int(doc), float(term)
                if doc not in lengths:
                    lengths[doc] = term
                else:
                    lengths[doc] += term
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        """
        d = defaultdict(list)
        for i, j in enumerate(docs, 1):
            for k in set(j):
                d[k].append([i, float(j.count(k))])
