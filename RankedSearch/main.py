        ...
        if line == '':
            ...
            for i in num:
                if i != '' and i != "\n":
                    words.append(i)
                    index = int(words[0])
                    result[index] = list(map(int, words[1:]))
    return result
...
def search(query, scorer, index):
    ...
    vector = index.query_to_vector(tokens)
    s = scorer.score(vector, index)
    return [k for k, v in sorted(s.items(), key=lambda x:round(x[1],6), reverse=True)]
