...
def tokenize(document):
    ...
    >>> tokenize("DOCTOR WHO!!  !!")
    ['doctor', 'who']
    >>> tokenize("#bestshow #EVER")
    ['bestshow', 'ever']
    """
    words = [x.lower() for x in re.findall('\w+',document)]
...
def create_index(tokens):
    ...
    for i in tokens:
        for j in i:
            if tokens.index(i) not in invIndex[j]:
                invIndex[j].append(tokens.index(i))
...
def sort_by_num_postings(words, index):
    ...
    ['c', 'a', 'b']
    >>> sort_by_num_postings(['a'], {'a': [0, 1]})
    ['a']
    >>> sort_by_num_postings([], {})
    []
    """
    countIndex = {}
    for i in words:
        countIndex[i] = len(index[i])
        
    sortedIndex = sorted(countIndex, key=countIndex.__getitem__)
...
def search(index, query):
    ...
    [1]
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4]}, 'd g')
    []
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [1, 2], 'e': [3, 4]}, 'a b d')
    [1]
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [1, 2], 'e': [3, 4]}, 'a b g')
    []
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [1, 2], 'e': [3, 4]}, 'a b b')
    [1]
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [0, 2], 'e': [3, 4]}, 'a b d')
    []
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [0, 2], 'e': [3, 4]}, 'a g d')
    []
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [0, 2], 'e': [3, 4]}, 'g a d')
    []
    >>> search({'a': [0, 1], 'b': [1, 2, 3], 'c': [4], 'd': [1, 2], 'e': [3, 4]}, 'd b a')
    [1]
    """
    #tokenize the query
    queryTokens = tokenize(query)

    #sort by num postings
    sortedPostings = sort_by_num_postings(index.keys(), index)

    #search for query tokens in sorted postings
    #returns list of matches according to tokens
    result = []
    for i in queryTokens:
        for x in sortedPostings:
            if i == x:
                result.append(index[i])
            
   
    if not result:                      #empty result list, queries not found
        return result               
    elif len(queryTokens) != len(result):   #one or more query tokens are not found
        return []
    else:
        #takes query result of i ^ query result of i+1
        intersected = result[0]
        for j in result[1:]:
            intersected = intersect(intersected, j)
